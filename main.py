...
# import rotate
# from rotate import right_cw, right_ccw, left_cw, left_ccw, front_cw, front_ccw, turn_to_right, turn_to_front, up_cw, up_ccw, down_cw, down_ccw
...
#     # Preprocess for contour detection (use value channel)
...
#     hierarchy = None
#     try:
#         _ret = cv2.findContours(gray, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
#         if len(_ret) == 3:
#             _, contours, hierarchy = _ret
#         else:
...
#                 # Color classification (tune these ranges for your cube)
#                 h, s, v = mean_hsv
#                 color_id = 0
#                 if s < 60 and v > 120:
#                     color_id = 1  # White
...
#                     color_id = 2  # Yellow
#                 elif (h < 10 or h > 160) and s > 80:
#                     color_id = 3  # Red
#                 elif 10 < h < 25 and s > 80:
#                     color_id = 4  # Orange
#                 elif 35 < h < 85 and s > 80:
#                     color_id = 5  # Green
#                 elif 85 < h < 130 and s > 80:
#                     color_id = 6  # Blue
#                 else:
#                     color_id = 0  # Unknown
#                 blob_color = np.array([color_id, h, s, v, x, y, w, h])
...
#     face = np.zeros(9, dtype=int)
#     if len(blob_colors) == 9:
#         for i in range(9):
#             face[i] = int(blob_colors[i][0])  # Assign color_id from HSV classification
#         print("[DEBUG] Detected face:", face)
#         print("[DEBUG] Blob colors (color_id, h, s, v, x, y, w, h):\n", blob_colors)
#         if np.count_nonzero(face) == 9:
#             return face, blob_colors
#         else:
#             return [0,0], blob_colors
#     else:
#         print("[DEBUG] Not enough blobs detected. blob_colors:", blob_colors)
...
#             # Ensure front_face is 2D for consistent indexing
#             if front_face is not None and front_face.ndim == 1:
#                 front_face = np.reshape(front_face, (1, -1))
#             mf = front_face[0,4]
#             print(front_face)
#             print(mf)
#             #print("Show Up Face")
...
#             if up_face is not None and up_face.ndim == 1:
#                 up_face = np.reshape(up_face, (1, -1))
...
#             if down_face is not None and down_face.ndim == 1:
#                 down_face = np.reshape(down_face, (1, -1))
#             start_time = datetime.now()
#             while True:
#                 if (datetime.now() - start_time).total_seconds() > 3:
#                     break
...
#             if right_face is not None and right_face.ndim == 1:
#                 right_face = np.reshape(right_face, (1, -1))
#             start_time = datetime.now()
#             while True:
#                 if (datetime.now() - start_time).total_seconds() > 3:
#                     break
...
#             if left_face is not None and left_face.ndim == 1:
#                 left_face = np.reshape(left_face, (1, -1))
#             start_time = datetime.now()
#             while True:
#                 if (datetime.now() - start_time).total_seconds() > 3:
#                     break
...
#             if back_face is not None and back_face.ndim == 1:
#                 back_face = np.reshape(back_face, (1, -1))
#             start_time = datetime.now()
#             while True:
#                 if (datetime.now() - start_time).total_seconds() > 3:
#                     break
